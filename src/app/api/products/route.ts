...
export async function POST(request: NextRequest) {
  try {
    ...
    const productPayload: any = {
      ...
      origin_region_ar: originRegionAr?.trim() || '',
      origin_region_en: originRegionEn?.trim() || '',
      ...
      sensory_profile: {
        ...
        package_weight: packageWeight?.trim() || '',
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const mergedSensoryProfile = {
      ...
      cost_price: calculatedCost,
      package_weight: packageWeight !== undefined ? (packageWeight?.trim() || '') : (existingProd?.sensory_profile?.package_weight || ''),
    ...
    const updatePayload: any = {
      ...
      low_stock_threshold: Number(lowStockThreshold),
      weight_grams: Number(weightGrams),
      origin_region_ar: originRegionAr !== undefined ? (originRegionAr?.trim() || '') : (existingProd?.origin_region_ar || ''),
      origin_region_en: originRegionEn !== undefined ? (originRegionEn?.trim() || '') : (existingProd?.origin_region_en || ''),
...
export function formatProductRow(data: any): Product {
  ...
  return {
    ...
    packageWeight: data.package_weight?.trim() || data.sensory_profile?.package_weight?.trim() || (data.sensory_profile?.package_weight === '' ? undefined : (data.weight_grams ? (data.weight_grams >= 1000 ? `${data.weight_grams / 1000} كجم` : `${data.weight_grams} جم`) : undefined)),

...
export function generateLuxuryEmailHtml(
  ...
  switch (type) {
    ...
    case 'vip_invitation':
      subject = 'زاد | دعوة حصرية لنخبة الأعضاء: حصاد السدر الملكي 2026';
      headline = 'دعوة خاصة لنخبة مقتني زاد';
      subheadline = 'إصدار محدود مرقم (200 برطمان فقط)';
      messageBody = `
        <p>سعادة <strong>${data.recipientName}</strong>،</p>
        <p>يسر دار زاد أن تفتح باب الحجز المبكر للمنتجات الطبيعية الحصرية من أجود المناحل الطبيعية، بنقاء تام ورطوبة فائقة الانخفاض.</p>
